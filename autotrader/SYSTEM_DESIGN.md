...
## 2026-07-16 增量：选股质量观察指标（T1+T2，只观察不影响交易）

设计原则（用户要求）：一切可视化、可解释；指标方向**预先声明**（外部证据 + 全年审计互证，
调研结论存对话记录），升级为过滤规则的标准 = ≥4-8 周 + 每桶样本≥60 + 高/低 VIX 两环境方向一致。

- 数据：watchlist 19 个特征列。免费层（Finviz 页面自带）：Perf 周/月/YTD/年、周波动、
  量比、价格、市值、归一化跌幅 z；日报打标（`_tag_*`，每步独立降级）：跳空/CLV/200日线/
  52周高（1年日线批量）、板块对照（SECTOR_ETF）、财报标签、做空比例、尾盘30分钟（分钟线
  扩展到候选前20）；nightly_runs 加 VIX3M 期限结构 + 榜单宽度/均跌。
- 可视化：「每晚决策」页指标分桶仪表盘（8 维度 × 命中率/均收益，悬停显示假设与证据来源）+
  逐票明细 9 列 + runs 表环境列。
- 核心假设速查：财报跌→差；CLV<0.2（收在最低附近）→好（反直觉）；z≥2→好（仅无消息时）；
  200日线上→好；独跌→差；高做空≥15%→差；尾盘加速跌→好；大幅跳空→差（待验证，前两晚小样本
  出现反向读数，勿早下结论）。
